...
import com.ec.deckxel.repository.PropietarioRepository;
...
public class PropietarioController {
	...
	public ResponseEntity<List<Propietario>> productos() {
		final HttpHeaders httpHeaders = new HttpHeaders();
		List<Propietario> respuesta = new ArrayList<>();
...
//		httpHeaders.setETag(HttpHeaders.ETAG);
		try {
			...
			respuesta = (List<Propietario>) propietarioRepository.findAll();
//			cfgPais = GlobalValue.LISTACFGPAIS;
		} catch (Exception e) {
			// TODO: handle exception
			System.out.println("ERROR catalogues " + e.getMessage());
			httpHeaders.add("STATUS", "0");
			return new ResponseEntity<List<Propietario>>(respuesta, httpHeaders, HttpStatus.INTERNAL_SERVER_ERROR);
		}
		httpHeaders.add("STATUS", "1");
		return new ResponseEntity<List<Propietario>>(respuesta, httpHeaders, HttpStatus.OK);
	}
